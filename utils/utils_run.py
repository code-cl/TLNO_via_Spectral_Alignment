...
def Train(args, data_array, task_type = None):
    ...
    if 'Transolver' in args['model_type']:
        ...
        if 'C_matrix' in args:
            from utils.NORM_plus_cbf import Model
        else:
            from utils.NORM_plus import Model
        model = Model(args).to(device)
        file_name = 'NORM_plus'
    ...
    if task_type=='Transfer':
        ...
        # 输出结果
        print(f"Successfully initialized parameters: {num_initialized}")
        print(f"Missing parameters: {num_missing}")
        print(f"Unexpected parameters: {num_unexpected}")
