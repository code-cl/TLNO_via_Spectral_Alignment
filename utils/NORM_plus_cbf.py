...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if transfer_matrix.shape[0] != num_lbos :
            raise ValueError("Please check 'transfer_matrix' !") 
        transfer_matrix = torch.Tensor(transfer_matrix).to(device)
        self.C_matrix = nn.Parameter(transfer_matrix)
...
class NO_Layer(nn.Module):
    ...
    def forward(self, fx, C_matrix):
        fx = self.Conv(self.ln_1(fx), self.lbo_bases, self.lbo_inv, C_matrix) + fx
...
class Convolution_block(nn.Module):
    ...
    def forward(self, x, LBO_MATRIX, LBO_INVER, C_matrix ):
        #  x : B N C
        x = self.SpectralConv(x, LBO_MATRIX, LBO_INVER, C_matrix) 
...
class Spectral_Conv(nn.Module):
    ...
    def forward(self, x, LBO_MATRIX, LBO_INVER, C_matrix):
        ...
        spectral_feature = (fx_mid.permute(0, 1, 3, 2) @ LBO_INVER @ C_matrix).permute(0, 1, 3, 2)
        ...
        out_x = LBO_MATRIX @ C_matrix @ out_spectral_feature
