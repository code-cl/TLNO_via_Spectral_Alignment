...
class DataSet:
    ...
    def load_data(self, args):
        ...
        if n_train + n_test > args['y_data'].shape[0]:
            raise ValueError("Please check 'load_data_target' !")
            
        X = args['nodes']
        
        f_train_mean = np.mean(np.reshape(f_train, (-1, h, w)), 0)
        f_train_std  = np.std(np.reshape(f_train, (-1, h, w)), 0)
        f_train_mean = np.reshape(f_train_mean, (-1, h, w, 1))
        f_train_std  = np.reshape(f_train_std, (-1, h, w, 1))
        
        F_train = np.reshape(f_train, (-1, h, w, 1))
        F_test  = np.reshape(f_test, (-1, h, w, 1))
    ...
    def load_data_target(self, args):
        ...
        h = args['target_h'] # mesh resolution
        w = args['target_w'] # mesh resolution
        ...
        F_train_t = np.reshape(f_train, (-1, h, w, 1))
        F_test_t  = np.reshape(f_test, (-1, h, w, 1))
