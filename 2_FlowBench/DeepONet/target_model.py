...
def main(args):
    ...
    f_ph = tf.placeholder(shape=[None, 1, x_bc], dtype=tf.float32) #[bs, 1, h, w, n_channels]
    ...
    while n <= nmax:
        ...
        if n%1 == 0:
            ...
            for i in range(ni):
                ...
                test_id, x_test, f_test, u_test = data.testbatch_target(bs, batch_id) # bs
    ...
    print('Total run time (secs):  %.3f'%(stop_time - start_time))
...
if __name__ == "__main__":
    ...
    geo_list = ['NURBS_10', 'NURBS_18']
    r_list = np.array([16263, 16257])
    source_geo = 0
    target_geo = 1
    n_i = 2
    ...
    data = io.loadmat(data_path + '/data.mat')
    x_data = data["input"][:,:,n_i]
    y_data = data["output"][:,:,n_i]
    x_data = np.pad(x_data, ((0, 0), (0, 6)), mode='constant', constant_values=0)
    ...
    for n_train in n_trains:
        ...
        target_save_path = "../logs/"+geo_list[source_geo]+'_'+geo_list[target_geo]+ '/' + str(n_i) + '/DeepONet_Ntr' + str(args['target_ntr'])\
                            + '_Nte' + str(args['target_nte'])

        for i, save_index in enumerate(save_indexs):
            source_folder = "../logs/"+geo_list[source_geo]+ '/' + str(n_i)+"/DeepONetBST_Ntr181_Nte60/" + str(save_index)
