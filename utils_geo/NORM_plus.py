...
class Model(nn.Module):
    def __init__(self, args):
        ...
        LBO_Base = args['lbo_bases'] 
        LBO_Inver = args['lbo_inver'] 
        for i in range(len(LBO_Base)):
            print('lbo_bases:', LBO_Base[i].shape, 'lbo_inver:', LBO_Inver[i].shape)
...
class NO_Layer(nn.Module):
    ...
    # def forward(self, fx, ids):
    #     no_geo  = ids[0,1]
    #     fx = self.Conv(self.ln_1(fx), self.lbo_bases[no_geo], self.lbo_inv[no_geo]) + fx
    #     fx = self.mlp(self.ln_2(fx)) + fx
    #     return fx
    
    def forward(self, fx, ids):
        yy = torch.zeros((fx.shape)).cuda()
        for i in range(fx.shape[0]):
            nodes = ids[i,0]
            x = fx[i:i+1, :nodes,:]
            lbo_bases = torch.Tensor(self.lbo_bases[ids[i,1]]).cuda()
            lbo_inver = torch.Tensor(self.lbo_inv[ids[i,1]]).cuda()
            x = self.Conv(self.ln_1(x), lbo_bases, lbo_inver) + x
