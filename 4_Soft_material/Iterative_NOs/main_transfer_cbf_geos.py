...
if __name__ == "__main__":
    ...
    save_path = '../logs_geos/' + source_shape + '→' + target_shape
    # Source_lbo_path = '../Data/' + source_shape + '/Nodes_LBO_basis'
    Target_lbo_path = '../Data/' + target_shape + '/Nodes_LBO_basis'
    
    source_model_str = 'NORM_plus_ntr15000_nte6000_L2_nl8_nm64_nc128_nh8_AdamW_OneCycleLR'
    source_model_path = '../logs_geos/' + source_shape + '/' + source_model_str
    ...
    for n_t in n_trains:
        ...
        args['C_matrix']  =  np.eye(args['num_lbos']) # C_matrix
        
        current_path = save_path + '/'+ args['model_type'] + '_ntr' + str(args['n_train']) + current_directory
        
        if np.array_equal(args['C_matrix'], np.eye(args['num_lbos'])):
